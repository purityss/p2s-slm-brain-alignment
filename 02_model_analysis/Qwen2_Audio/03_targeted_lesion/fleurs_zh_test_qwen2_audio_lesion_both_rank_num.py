...
# save_folder = "/home/user/model_activate/qwen_audio/lesion_rank_results"
...
# === Read neuron data from .mat file and sort by the sum of significance values ===
mat = io.loadmat("qwen2_audio_start_both_neurons.mat")
neuron_data = mat["start_both_neurons"]

valid_neurons = [(int(row[0]) - 1, int(row[1]) - 1, float(row[3]) + float(row[4]))
...
# save_path = os.path.join(save_folder, "sorted_start_both_neurons.mat")
# io.savemat(save_path,  {"sorted_neurons": sorted_array})
# print("✅ Sorted neuron information has been saved as sorted_start_both_neurons.mat")

results = []
# total_neurons = len(sorted_neurons)
total_neurons = 1767

# Modify here: from top 0, 200, 400... up to a maximum of 1767
step = 100
...
save_path = os.path.join(save_folder, "qwen2_audio_lesion_rank_results_both_num.mat")
