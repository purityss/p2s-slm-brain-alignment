...
% save_fig_name_1 = 'llasm_mds_sample_1';
save_fig_name_2 = 'llasm_distance_sem_2';
name = 'llasm';
load(strcat(name, '_sem_avg_token_rdm.mat'));
token = 2;  % {1,2}
data = sem_avg_token_rdm{token};
...
mean_tongyin = nanmean(elements_tongyin); SE_tongyin = std(elements_tongyin)/sqrt(length(elements_tongyin));
mean_yiyin = nanmean(elements_yiyin); SE_yiyin = std(elements_yiyin)/sqrt(length(elements_yiyin));
...
mean_tongyi = nanmean(elements_tongyi); SE_tongyi = std(elements_tongyi)/sqrt(length(elements_tongyi));
mean_yiyi = nanmean(elements_yiyi); SE_yiyi = std(elements_yiyi)/sqrt(length(elements_yiyi));
