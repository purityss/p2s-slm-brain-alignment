...
save_fig_name = 'qwen2_audio_mds_flow_sem';
name = 'qwen2_audio';

% --- 加载数据 ---
% 加载包含两个 token RDM 的数据文件
load(strcat(name, '_sem_avg_token_rdm.mat')); 
% 变量名应该是 sem_avg_token_rdm，是一个 1x2 的 cell
...
for t_idx = 1:num_time_points
    ...
    % 根据你的第一段代码逻辑：data = sem_avg_token_rdm{token};
    mean_rdm = sem_avg_token_rdm{token_idx};
