...
save_fig_path = 'G:\Research\Auditory model\Figure_AI\Sup4_part';
% save_fig_name_1 = 'mds_sample_2';
save_fig_name_2 = 'distance_sample_12';

data_path = 'F:\code\Matlab work\code_for_SEEG_data\code_for_all_subjects\selective_contacts_step_rdm_not_control\all_sub_selective_contacts\MDS_RDM\pho_rdm_all_time\'; % 数据文件存放路径
...
if h == 1
    if p < 0.001
        text((x1+x2)/2, height_phon + 0.001, '***', 'FontSize', 10, 'HorizontalAlignment', 'center');
    elseif p < 0.01
        text((x1+x2)/2, height_phon + 0.001, '**', 'FontSize', 10, 'HorizontalAlignment', 'center');
    elseif p < 0.05
        text((x1+x2)/2, height_phon + 0.001, '*', 'FontSize', 10, 'HorizontalAlignment', 'center');
        ...
        text((x1+x2)/2, height_phon + 0.002, 'n.s.', 'FontSize', 8, 'HorizontalAlignment', 'center');
end

% ylabel('Phonological distance','FontSize', 8, 'FontName', 'Arial', 'Color', 'k');
ylim([0.84 0.88]); yticks(0.84:0.04:0.88);
% ax.YTick = [];       % 去除所有刻度
...
if h == 1
    if p < 0.001
        text((x3+x4)/2, height_sem + 0.001, '***', 'FontSize', 10, 'HorizontalAlignment', 'center');
    elseif p < 0.01
        text((x3+x4)/2, height_sem + 0.001, '**', 'FontSize', 10, 'HorizontalAlignment', 'center');
    elseif p < 0.05
        text((x3+x4)/2, height_sem + 0.001, '*', 'FontSize', 10, 'HorizontalAlignment', 'center');
        ...
        text((x3+x4)/2, height_sem + 0.002, 'n.s.', 'FontSize', 8, 'HorizontalAlignment', 'center');
end

% ylabel('Semantic distance','FontSize', 8, 'FontName', 'Arial', 'Color', 'k');
ylim([0.84 0.88]); yticks(0.84:0.04:0.88);
% ax.YTick = [];       % 去除所有刻度
%% ========== 美化图表 ==========
xticks([x1, x2, x3, x4]);
