...
% Layout parameters
time_spacing = 1.5;  % Horizontal gap between time snapshots
x_compression = 0.4; % MDS horizontal compression factor
...
% Styles
color_light_blue = [0, 173, 181]/255; % Green-themed category
color_dark_blue = [240/255, 167/255, 58/255]; % Yellow-themed category
normal_marker_size = 10; 
special_marker_size = 50;

% Highlighted points config
specific_markers = {
    4,  's', 'light'; 
    30, 's', 'dark';  
    20, 'o', 'light'; 
...
for t = 1:num_time_points
    for i = 1:num_words
        ...
        if is_special
            ...
            % Draw special shapes without black borders
            if strcmp(shape, 'o')
...
for t = 1:num_time_points
    ...
    rectangle('Position', [center_offset - ellipse_w/2, -ellipse_h/2, ellipse_w, ellipse_h], ...
              'Curvature', [1 1], 'EdgeColor', 'k', 'LineStyle', '-', 'LineWidth', 0.5);
end
