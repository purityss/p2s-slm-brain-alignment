...
save_fig_name = sprintf('%s_combined_t_heatmap_line', name);
...
mA1 = nanmean(Line_A(1:p2, :), 1);
mB1 = nanmean(Line_B(1:p2, :), 1);

mA2 = nanmean(Line_A(b1:b2, :), 1);
mB2 = nanmean(Line_B(b1:b2, :), 1);

mA3 = nanmean(Line_A(s1:s2, :), 1);
mB3 = nanmean(Line_B(s1:s2, :), 1);
...
for k = 1:3
    ...
    % 增加 YLabel 以占据左侧空间 (如果不需要内容可设为空格 ' ')
    % ylabel('Neurons', 'FontWeight', 'normal'); 
    % ylabel(''); % 如果不想显示文字只想占位，可以取消注释这行
    
    % 增加标题以拉大垂直间距
    % title(row_titles{k}, 'FontSize', 9, 'FontWeight', 'bold');
    ...
    hold on; % 【关键修改】：保持绘图状态，使两条线画在同一个轴上
    
    % 画第一条线 (Phonological - 红)
    ...
    % 标题保持对齐
    t_obj2 = title(row_titles{k}, 'FontSize', 11, 'Color', 'w'); % 白色标题仅占位
